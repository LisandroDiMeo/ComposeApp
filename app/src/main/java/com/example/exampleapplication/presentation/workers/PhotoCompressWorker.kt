package com.example.exampleapplication.presentation.workers
...
class PhotoCompressWorker(
    ...
    override suspend fun doWork(): Result {
        return withContext(Dispatchers.IO) {
            val imageUriString = workerParameters.inputData.getString(KEY_CONTENT_URI)
            val compressionThresholdInBytes = workerParameters
                .inputData
                .getLong(KEY_COMPRESSION_THRESHOLD, 0L)
            val imageUri = Uri.parse(imageUriString)
            // Remember to use "use" in streams, to automatically close them.
            ...
            val file = File(applicationContext.cacheDir, "${workerParameters.id}.jpg")
